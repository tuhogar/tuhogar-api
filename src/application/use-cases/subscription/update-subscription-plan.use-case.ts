...
export class UpdateSubscriptionPlanUseCase {
  ...
  async execute({
    ...
    planId,
    paymentData,
  }: UpdateSubscriptionPlanUseCaseCommand): Promise<Subscription> {
    const account =
      await this.accountRepository.findOneByIdWithPaymentData(accountId);
    if (!account) throw new Error('invalid.account.do.not.exists');

    const actualSubscription =
      await this.subscriptionRepository.findOneWithResultIntegrationById(
        actualSubscriptionId,
      );
    if (!actualSubscription)
      throw new Error('invalid.subscription.do.not.exists');

    if (
      actualSubscription.status !== SubscriptionStatus.ACTIVE ||
      actualSubscription.planId === this.firstSubscriptionPlanId
    )
      throw new Error('invalid.subscription.do.not.active');
    ...
    try {
      const customer = await this.paymentGateway.getCustomer(
        actualSubscription.externalPayerReference,
      );
      ...
      const actualSubscriptionPlanIdIndex = planIds.indexOf(
        actualSubscription.planId,
      );
      const planIdIndex = planIds.indexOf(planId);

      if (actualSubscriptionPlanIdIndex === -1 || planIdIndex === -1)
        throw new Error('invalid.plan.do.not.exists');

      const downgradeOrUpgrade =
        planIdIndex < actualSubscriptionPlanIdIndex ? 'DOWNGRADE' : 'UPGRADE';

      if (downgradeOrUpgrade === 'UPGRADE') {
        await this.cancelSubscriptionOnPaymentGatewayUseCase.execute({

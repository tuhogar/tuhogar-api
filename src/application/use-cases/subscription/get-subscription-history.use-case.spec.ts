...
describe('GetSubscriptionHistoryUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<GetSubscriptionHistoryUseCase>(
      GetSubscriptionHistoryUseCase,
    );
    subscriptionRepository = module.get<ISubscriptionRepository>(
      ISubscriptionRepository,
    );
    subscriptionPaymentRepository = module.get<ISubscriptionPaymentRepository>(
  ...
  describe('execute', () => {
    it('should return all subscriptions with their payments', async () => {
      ...
      jest
        .spyOn(subscriptionRepository, 'findAllByAccountId')
        .mockResolvedValue(mockSubscriptions);

      // Mock para retornar os pagamentos para cada assinatura
      jest
        .spyOn(subscriptionPaymentRepository, 'findAllBySubscriptionId')
      ...
      expect(subscriptionRepository.findAllByAccountId).toHaveBeenCalledWith(
        accountId,
      );
      expect(
        subscriptionPaymentRepository.findAllBySubscriptionId,
      ).toHaveBeenCalledWith('subscription-id-1');
      expect(
        subscriptionPaymentRepository.findAllBySubscriptionId,
      ).toHaveBeenCalledWith('subscription-id-2');
      expect(
        subscriptionPaymentRepository.findAllBySubscriptionId,
    ...
    it('should throw an error when no subscriptions are found', async () => {
      ...
      await expect(useCase.execute({ accountId })).rejects.toThrow(
        'notfound.subscription.history.do.not.exists',
      );

...
export class ReceiveSubscriptionPaymentNotificationUseCase {
  ...
  constructor(
    ...
    const subscriptionTotalDays = this.configService.get<string>(
      'SUBSCRIPTION_TOTAL_DAYS',
    );
    this.subscriptionTotalDays = subscriptionTotalDays
      ? parseInt(subscriptionTotalDays, 10)
      : 30; // Valor padrão de 30 dias se não for definido
  }

  async execute(
    subscriptionNotification: SubscriptionNotification,
  ): Promise<void> {
    const paymentNotificated = await this.paymentGateway.getPayment(
      subscriptionNotification,
    );
    ...
    if (paymentNotificated.externalSubscriptionReference) {
      subscription = await this.subscriptionRepository.findOneByExternalId(
        paymentNotificated.externalSubscriptionReference,
      );
    } else if (paymentNotificated.externalPayerReference) {
      subscription =
        await this.subscriptionRepository.findOneByExternalPayerReference(
          paymentNotificated.externalPayerReference,
        );
    }
    ...
    const payment =
      await this.subscriptionPaymentRepository.findOneByExternalId(
        paymentNotificated.externalId,
      );
    ...
    if (paymentNotificated.status === SubscriptionPaymentStatus.APPROVED) {
      if (
        ...
        subscription.status !== SubscriptionStatus.CANCELLED_ON_PAYMENT_GATEWAY
      ) {
        ...
        if (
          actualSubscription &&
          actualSubscription.planId === this.firstSubscriptionPlanId
        )
          await this.subscriptionRepository.cancel(actualSubscription.id);
      }

      // Gravar a data do pagamento na assinatura
      if (paymentNotificated.paymentDate && subscription) {
        console.log(
          `Atualizando data de pagamento da assinatura ${subscription.id}`,
        );
        await this.subscriptionRepository.updatePaymentDate(
          subscription.id,
          paymentNotificated.paymentDate,
        );
        ...
        nextPaymentDate.setDate(
          nextPaymentDate.getDate() + this.subscriptionTotalDays,
        );
        console.log(
          `Atualizando data do próximo pagamento da assinatura ${subscription.id} para ${nextPaymentDate.toISOString()}`,
        );
        await this.subscriptionRepository.updateNextPaymentDate(
          subscription.id,
          nextPaymentDate,
        );
      }

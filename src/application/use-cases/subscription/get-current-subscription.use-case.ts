...
export class GetCurrentSubscriptionUseCase {
  ...
  async execute({
    accountId,
  }: GetCurrentSubscriptionUseCaseCommand): Promise<SubscriptionWithRemainingFreeDays> {
    ...
    if (
      ...
      subscription.plan.freeTrialDays > 0
    ) {
      ...
      remainingFreeDays =
        daysSinceCreation >= totalFreeDays
          ? 0
          : totalFreeDays - daysSinceCreation;
    }

    const typeDocumentCoupon =
      await this.accountCouponRepository.findTypeDocumentCouponByAccountId(
        accountId,
      );

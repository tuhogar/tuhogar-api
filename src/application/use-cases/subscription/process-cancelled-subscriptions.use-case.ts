...
export class ProcessCancelledSubscriptionsUseCase {
  private readonly logger = new Logger(
    ProcessCancelledSubscriptionsUseCase.name,
  );
  ...
  async executeScheduled(): Promise<void> {
    try {
      ...
      this.logger.error(
        `Erro no processamento automático de assinaturas canceladas: ${error.message}`,
      );
  ...
  async execute(): Promise<void> {
    ...
    const subscriptionsToCancel =
      await this.subscriptionRepository.findSubscriptionsToCancel(currentDate);
  ...
  private async processSubscription(subscription: Subscription): Promise<void> {
    try {
      this.logger.log(
        `Processando cancelamento de assinatura ${subscription.id} da conta ${subscription.accountId}`,
      );
      ...
      await this.accountRepository.updatePlan(
        subscription.accountId,
      ...
      this.logger.log(
        `Cancelamento de assinatura ${subscription.id} processada com sucesso`,
      );
    } catch (error) {
      this.logger.error(
        `Erro ao processar cancelamento de assinatura ${subscription.id}: ${error.message}`,
      );

...
export class ReceiveSubscriptionInvoiceNotificationUseCase {
  ...
  async execute(
    ...
    const invoiceNotificated = await this.paymentGateway.getInvoice(
      subscriptionNotification,
    );
    ...
    const subscription = await this.subscriptionRepository.findOneByExternalId(
      invoiceNotificated.externalSubscriptionReference,
    );
    if (!subscription) {
    ...
    const invoice =
      await this.subscriptionInvoiceRepository.findOneByExternalId(
        invoiceNotificated.externalId,
      );

...
describe('ReceiveSubscriptionPaymentNotificationUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    configService = {
      get: jest.fn().mockImplementation((key: string) => {
        if (key === 'FIRST_SUBSCRIPTION_PLAN_ID')
          return mockFirstSubscriptionPlanId;
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ISubscriptionRepository, useValue: subscriptionRepository },
        {
          ...
          useValue: subscriptionPaymentRepository,
        },
        {
          provide: ISubscriptionNotificationRepository,
          useValue: subscriptionNotificationRepository,
        },
    ...
    useCase = module.get<ReceiveSubscriptionPaymentNotificationUseCase>(
      ReceiveSubscriptionPaymentNotificationUseCase,
    );
  });
  ...
  it('should process an approved payment notification and update payment date and nextPaymentDate', async () => {
    ...
    subscriptionRepository.findOneByExternalId.mockResolvedValue(
      mockSubscription,
    );
    ...
    mockNextPaymentDate.setDate(
      mockNextPaymentDate.getDate() + parseInt(mockSubscriptionTotalDays, 10),
    );
    ...
    expect(subscriptionRepository.findOneByExternalId).toHaveBeenCalledWith(
      mockExternalSubscriptionReference,
    );
    ...
    expect(subscriptionRepository.updatePaymentDate).toHaveBeenCalledWith(
      mockSubscriptionId,
      mockPaymentDate,
    );

    // Verificar a atualização da data do próximo pagamento
    expect(subscriptionRepository.updateNextPaymentDate).toHaveBeenCalledWith(
      mockSubscriptionId,
      expect.any(Date),
    );

    // Verificar que a data do próximo pagamento foi atualizada
    const actualNextPaymentDate =
      subscriptionRepository.updateNextPaymentDate.mock.calls[0][1];
  ...
  it('should update existing payment when notification action is not CREATE', async () => {
    ...
    subscriptionPaymentRepository.findOneByExternalId.mockResolvedValue(
      mockPayment,
    );
  ...
  it('should find subscription by external payer reference if external subscription reference is not available', async () => {
    ...
    subscriptionRepository.findOneByExternalPayerReference.mockResolvedValue(
      mockSubscription,
    );
    subscriptionPaymentRepository.findOneByExternalId.mockResolvedValue(null);
    subscriptionPaymentRepository.create.mockResolvedValue(
      paymentWithoutSubRef,
    );
    subscriptionRepository.active.mockResolvedValue(mockSubscription);
    subscriptionRepository.findOneActiveByAccountId.mockResolvedValue(null);
    subscriptionRepository.updatePaymentDate.mockResolvedValue({
    ...
    expect(paymentGateway.getPayment).toHaveBeenCalledWith(
      notificationWithoutSubRef,
    );
    expect(subscriptionRepository.findOneByExternalId).not.toHaveBeenCalled();
    expect(
      subscriptionRepository.findOneByExternalPayerReference,
    ).toHaveBeenCalledWith(mockExternalPayerReference);
    expect(subscriptionPaymentRepository.create).toHaveBeenCalled();
    expect(subscriptionRepository.active).toHaveBeenCalledWith(
      mockSubscriptionId,
    );

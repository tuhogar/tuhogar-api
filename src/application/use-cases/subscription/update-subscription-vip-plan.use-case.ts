...
export class UpdateSubscriptionVipPlanUseCase {
  ...
  async execute({
    ...
    planId,
    nextPaymentDate,
  }: UpdateSubscriptionVipPlanUseCaseCommand): Promise<Subscription> {
    // Busca a assinatura mais recente do usuário
    const subscription =
      await this.subscriptionRepository.findMostRecentByAccountId(accountId);
    ...
    try {
      const subscriptionUpdated = await this.subscriptionRepository.updatePlan(
        subscription.id,
        planId,
      );
      if (nextPaymentDate) {
        await this.subscriptionRepository.updateNextPaymentDate(
          ...
          new Date(
            Date.UTC(
              parseInt(nextPaymentDate.substring(0, 4)),
              parseInt(nextPaymentDate.substring(4, 6)) - 1,
              parseInt(nextPaymentDate.substring(6, 8)),
            ),
          ),
        );
      }

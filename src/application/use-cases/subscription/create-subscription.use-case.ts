...
export class CreateSubscriptionUseCase {
  ...
  async execute({
    ...
    paymentData,
    customerId,
  }: CreateSubscriptionUseCaseCommand): Promise<Subscription> {
    ...
    if (account.hasPaidPlan && plan.freeTrialDays > 0)
      throw new Error('invalid.subscription.plan');

    const accountCoupon =
      await this.accountCouponRepository.findLastNotDepletedByAccountId(
        accountId,
      );
    ...
    if (coupon) {
      if (coupon.expirationDate && coupon.expirationDate < new Date())
        throw new Error('invalid.coupon.expiredDate');
    ...
    try {
      const { subscription: externalSubscriptionCreated, customer } =
      ...
      if (planId !== this.firstSubscriptionPlanId) {
        ...
        console.log(
          `Conta ${accountId} marcada como tendo assinado um plano pago`,
        );
      }

      if (setCouponDepleted)
        await this.accountCouponRepository.deplete(accountCoupon.id);
      ...
      console.log(error);
      console.log('-------------error-on-create-subscription-------------');
      await this.removeInternalSubscriptionUseCase.execute({

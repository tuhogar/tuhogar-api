...
export class GetAccountAdvertisementStatisticsUseCase {
  ...
  async getByMonth(
    ...
    if (!statistics) {
      throw new Error(
        'notfound.account.advertisement.statistics.do.not.exists',
      );
    }
    ...
    if (advertisementIds.length > 0) {
      ...
      statistics.topViewedAdvertisements.sale.forEach((advertisement) => {
        advertisement.advertisement = advertisements.find(
          (ad) => ad.id === advertisement.advertisementId,
        );
      });
      statistics.topViewedAdvertisements.rent.forEach((advertisement) => {
        advertisement.advertisement = advertisements.find(
  ...
  async getAllMonthsByAccount(accountId: string): Promise<string[]> {
    const allMonths =
      await this.accountAdvertisementStatisticsRepository.findAllMonthsByAccountId(
        accountId,
      );

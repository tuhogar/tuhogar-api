...
export class CreateAdvertisementEventUseCase {
  ...
  async execute({
    advertisementId,
    type,
  }: CreateAdvertisementEventUseCaseCommand): Promise<AdvertisementEvent> {
    const exists =
      await this.advertisementEventRepository.findOneByAdvertisementIdAndType(
        advertisementId,
        type,
      );
    if (exists) {
      return this.advertisementEventRepository.update(
        exists.id,
        exists.count + 1,
      );
    }
    ...
    const response =
      await this.advertisementEventRepository.create(advertisementEvent);
    return response;
  }
}

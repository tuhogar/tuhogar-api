...
export class ProcessImagesAdvertisementUseCase {
  ...
  async execute(
    ...
    uploadImagesAdvertisementDto: UploadImagesAdvertisementDto,
    ...
    if (authenticatedUser.userRole === UserRole.MASTER) {
      ...
      const account = await this.accountRepository.findOneByIdWithSubscription(
    ...
    if (maxPhotos !== undefined && maxPhotos !== null && maxPhotos > 0) {
      const currentPhotosCount = advertisement.photos
    ...
    for (const image of uploadImagesAdvertisementDto.images) {
      ...
      const resizedImageContent = await this.cloudinaryService.resizeImage(
        image.content,
        1920,
        1080,
      );
      ...
      const imageUrl = await this.cloudinaryService.uploadBase64Image(
        ...
        'image/webp',
        imageName,
        `advertisements${this.cloudinaryFolders}`,
      );
      ...
      const imageThumbnailUrl = imageUrlStr.replace(
        'upload/',
        'upload/c_thumb,w_352,h_352,c_fill,ar_1.0/',
      );
    ...
    const updatedAdvertisement =
      await this.advertisementRepository.createPhotos(
        authenticatedUser.userRole !== UserRole.MASTER

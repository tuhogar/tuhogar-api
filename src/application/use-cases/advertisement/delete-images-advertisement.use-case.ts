...
export class DeleteImagesAdvertisementUseCase {
  ...
  async execute(
    ...
    deleteImagesAdvertisementDto: DeleteImagesAdvertisementDto,
  ): Promise<void> {
    const advertisement =
    ...
    await this.advertisementRepository.updatePhotos(
      authenticatedUser.userRole !== UserRole.MASTER
    ...
    await Promise.all([
      ...
      photosToRemove.map((a) =>
        this.cloudinaryService.deleteImage(this.getPublicIdFromImageUrl(a.url)),
      ),

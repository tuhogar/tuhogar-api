...
export class AdjustAdvertisementsAfterPlanChangeUseCase {
  private readonly logger = new Logger(
    AdjustAdvertisementsAfterPlanChangeUseCase.name,
  );
  ...
  async execute({
    accountId,
    planId,
  }: AdjustAdvertisementsAfterPlanChangeUseCaseCommand): Promise<{
    pausedDueToPhotoLimit: number;
    pausedDueToAdvertisementLimit: number;
    totalActiveAdvertisements: number;
  }> {
    this.logger.log(
      `Iniciando ajuste automático de anúncios para accountId: ${accountId}`,
    );
    ...
    try {
      ...
      if (
        plan.maxPhotos === undefined ||
        plan.maxAdvertisements === undefined
      ) {
      ...
      this.logger.log(
        `Verificando anúncios com excesso de fotos para accountId: ${accountId}`,
      );

      // Buscar anúncios ativos ou aguardando aprovação com excesso de fotos
      const advertisementsWithExcessPhotos =
        await this.advertisementRepository.findActiveOrWaitingWithExcessPhotos(
          accountId,
          plan.maxPhotos,
        );
      ...
      if (advertisementsWithExcessPhotos.length > 0) {
        this.logger.log(
          `Encontrados ${advertisementsWithExcessPhotos.length} anúncios com excesso de fotos`,
        );
        ...
        this.logger.log(
          `${pausedDueToPhotoLimit} anúncios pausados por excederem o limite de fotos`,
        );
      } else {
      ...
      this.logger.log(
        `Verificando limite de anúncios ativos para accountId: ${accountId}`,
      );
      ...
      if (plan.maxAdvertisements > 0) {
        ...
        const activeAdvertisements =
          await this.advertisementRepository.findActiveOrWaitingByAccountIdWithOrder(
            accountId,
        ...
        if (activeAdvertisements.length > plan.maxAdvertisements) {
          this.logger.log(
            `Encontrados ${activeAdvertisements.length} anúncios ativos, limite do plano: ${plan.maxAdvertisements}`,
          );

          // Calcular quantos anúncios precisam ser pausados
          const excessAdvertisements =
            activeAdvertisements.length - plan.maxAdvertisements;

          // Selecionar os anúncios mais recentes para pausar (os primeiros da lista, já que está ordenada por data decrescente)
          const advertisementsToPause = activeAdvertisements.slice(
            0,
            excessAdvertisements,
          );
          ...
          this.logger.log(
            `${pausedDueToAdvertisementLimit} anúncios mais recentes pausados por excederem o limite de anúncios ativos`,
          );
        } else {
          this.logger.log(
            `Número de anúncios ativos (${activeAdvertisements.length}) está dentro do limite do plano (${plan.maxAdvertisements})`,
          );
        }
      } else {
        this.logger.log(
          `Plano sem limite de anúncios ativos ou com limite zero`,
        );
      }

      // Obter número total de anúncios ativos após o ajuste
      const totalActiveAdvertisements =
        await this.advertisementRepository.countActiveOrWaitingByAccountId(
          accountId,
        );

      this.logger.log(
        `Ajuste automático de anúncios concluído com sucesso para accountId: ${accountId}`,
      );
      ...
      this.logger.error(
        `Erro durante o ajuste automático de anúncios: ${error.message}`,
      );
      throw error;
    }
  }

...
export class BulkAdvertisementUseCase {
  ...
  async execute(
    bulkAdvertisementUseCaseCommand: BulkAdvertisementUseCaseCommand,
  ): Promise<void> {
    let lastUpdatedAt = undefined;

    if (!bulkAdvertisementUseCaseCommand?.accountId) {
      lastUpdatedAt =
        (await this.bulkUpdateDateRepository.findOne())?.updatedAt ||
        new Date(0);
    }

    const advertisements = await this.advertisementRepository.findForBulk(
      bulkAdvertisementUseCaseCommand?.accountId,
      lastUpdatedAt,
    );

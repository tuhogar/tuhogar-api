...
describe('ProcessImagesAdvertisementUseCase', () => {
  ...
  const mockAdvertisement: Advertisement = {
    ...
    communityAmenities: [],
    description:
      'Hermoso apartamento en El Poblado, con vista panorámica a la ciudad. Excelente ubicación cerca de centros comerciales, restaurantes y transporte público. Ideal para inversión o vivienda.',
    hoaFee: 350000,
    lotArea: 120,
    floorArea: 95,
    price: 450000000,
  ...
  beforeEach(async () => {
    ...
    const mockCloudinaryService = {
      ...
      uploadBase64Image: jest
        .fn()
        .mockImplementation((content, type, name) =>
    ...
    const mockAdvertisementRepository = {
      ...
      createPhotos: jest
        .fn()
        .mockImplementation((accountId, advertisementId, photos) => {
    ...
    useCase = module.get<ProcessImagesAdvertisementUseCase>(
      ProcessImagesAdvertisementUseCase,
    );
    algoliaService = module.get<AlgoliaService>(AlgoliaService);
    cloudinaryService = module.get<CloudinaryService>(CloudinaryService);
    advertisementRepository = module.get<IAdvertisementRepository>(
  ...
  describe('Validação de limite de fotos', () => {
    it('deve permitir o upload de fotos quando o usuário não atingiu o limite', async () => {
      ...
      jest
        .spyOn(advertisementRepository, 'findOneById')
        .mockResolvedValue(mockAdvertisement); // 0 fotos existentes

      // Act & Assert
      await expect(
        useCase.execute(mockAuthenticatedUser, mockAdvertisementId, uploadDto),
    ...
    it('deve permitir o upload de fotos quando o total não excede o limite', async () => {
      ...
      jest
        .spyOn(advertisementRepository, 'findOneById')
        .mockResolvedValue(mockAdvertisementWithPhotos); // 3 fotos existentes
    ...
    it('não deve validar o limite quando maxPhotos é undefined', async () => {
      ...
      // Act & Assert
      await expect(
        useCase.execute(userWithoutMaxPhotos, mockAdvertisementId, uploadDto),
      ).resolves.toBeDefined();
    });

    it('não deve validar o limite quando maxPhotos é null', async () => {
      // Arrange
      const userWithNullMaxPhotos = {
        ...mockAuthenticatedUser,
        maxPhotos: null,
      };
      const uploadDto = createMockUploadDto(10); // 10 novas fotos (mais que o limite)
      ...
      // Act & Assert
      await expect(
        useCase.execute(userWithNullMaxPhotos, mockAdvertisementId, uploadDto),
      ).resolves.toBeDefined();
    });
    ...
    it('deve permitir o upload de uma foto quando maxPhotos é 0 e não há fotos existentes', async () => {
      ...
      jest
        ...
        .mockResolvedValue(mockAdvertisement); // 0 fotos existentes
      jest
        .spyOn(cloudinaryService, 'uploadBase64Image')
      ...
      expect(advertisementRepository.findOneById).toHaveBeenCalledWith(
        mockAdvertisementId,
      );
    ...
    it('deve permitir o upload de uma foto quando maxPhotos é 0 e já existem fotos', async () => {
      ...
      const mockAdvWithPhotos = {
        ...mockAdvertisement,
        photos: [existingPhoto],
      };
      jest
        .spyOn(advertisementRepository, 'findOneById')
        .mockResolvedValue(mockAdvWithPhotos);
      jest
        .spyOn(cloudinaryService, 'uploadBase64Image')
        .mockResolvedValue('https://example.com/new-image.jpg');
  ...
  describe('Validação de limite de fotos para usuários MASTER', () => {
    it('deve buscar o plano da conta do anúncio para usuários MASTER', async () => {
      ...
      jest
        .spyOn(advertisementRepository, 'findOneById')
        .mockResolvedValue(mockAdvertisement); // 0 fotos existentes
      const findOneByIdWithSubscriptionSpy = jest.spyOn(
        accountRepository,
        'findOneByIdWithSubscription',
      );
      ...
      // Assert
      expect(findOneByIdWithSubscriptionSpy).toHaveBeenCalledWith(
        mockAccountId,
      );
    ...
    it('deve lançar erro quando a conta do anúncio não é encontrada', async () => {
      ...
      jest
        ...
        .mockResolvedValue(mockAdvertisement); // 0 fotos existentes
      jest
        .spyOn(accountRepository, 'findOneByIdWithSubscription')
        .mockResolvedValue(null);

      // Act & Assert
      await expect(
        useCase.execute(mockMasterUser, mockAdvertisementId, uploadDto),
    ...
    it('deve tratar erro ao buscar o plano e não validar o limite', async () => {
      ...
      jest
        ...
        .mockResolvedValue(mockAccount);
      jest
        .spyOn(planRepository, 'findOneById')
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'error.fetching.plan.for.photos.limit.validation',
      );
    });
    ...
    it('não deve validar o limite quando a conta não tem plano associado', async () => {
      ...
      const accountWithoutPlan = {
        ...mockAccount,
        subscription: { ...mockAccount.subscription, planId: null },
      };
      const uploadDto = createMockUploadDto(10); // 10 novas fotos (mais que o limite)
      jest
        .spyOn(advertisementRepository, 'findOneById')
      ...
      jest
        .spyOn(accountRepository, 'findOneByIdWithSubscription')
        .mockResolvedValue(accountWithoutPlan);

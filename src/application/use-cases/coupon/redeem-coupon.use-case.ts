...
export class RedeemCouponUseCase {
  ...
  async execute({
    coupon,
    accountId,
  }: RedeemCouponUseCaseCommand): Promise<Coupon> {
    ...
    if (couponExists.expirationDate && couponExists.expirationDate < new Date())
      throw new Error('invalid.coupon.expirednDate');
    if (couponExists.isSingleRedemption && couponExists.isRedeemed)
      throw new Error('invalid.coupon.redeemed');

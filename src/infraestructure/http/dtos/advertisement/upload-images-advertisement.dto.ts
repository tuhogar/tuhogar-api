...
class UploadImageAdvertisementDto {
  ...
  @Matches(/^(image\/jpeg|image\/jpg|image\/webp|image\/png)$/, {
    message:
      'invalid.contentType.must.be.one.of.the.following.types.image/jpeg.image/jpg.image/webp.image/png',
  })
  contentType: string;
  ...
  @IsNumber(
    ...
    {
      message:
        'invalid.order.must.be.a.number.conforming.to.the.specified.constraints',
    },
  )
  order: number = 0;
}

...
export class EPaycoService implements IPaymentGateway {
  ...
  async createSubscription(accountId: string, subscriptionId: string, email: string, name: string, plan: Plan, paymentData: any): Promise<Subscription> {
    try {
      ...
      if (!customer.success) {
        ...
        throw new Error(`invalid.subscription.create.customer.creation.failed`);
      }
      ...
      if (!subscriptionResult.success) {
        ...
        throw new Error(`invalid.subscription.create.subscription.creation.failed`);
      }
          ...
          if (charge.data && charge.data.estado) {
            switch (charge.data.estado) {
              ...
              case 'Fallida':
                ...
                throw new Error(`invalid.subscription.create.payment.creation.failed`);
            }
          }
  ...
  async cancelSubscription(subscriptionId: string): Promise<any> {
    ...
    if (!result.status) {
      throw new Error('invalid.subscription.cancel.failed');
    }
  ...
  async cancelSubscriptionOnInvalidCreate(subscriptionId: string): Promise<any> {
    ...
    if (!result.status) {
      throw new Error('invalid.subscription.create.cancel.on.invalid.create.failed');
    }

...
export class MongooseCouponMapper {
  static toDomain(entity: CouponDocument): Coupon {
    ...
    const model = new Coupon({
      ...
      doesNotHavePaidPlanIds: !!entity.doesNotHavePaidPlanIds
        ? entity.doesNotHavePaidPlanIds.map((a) =>
            MongoosePlanMapper.toDomain(a),
          )
        : undefined,
      hasPaidPlanIds: !!entity.hasPaidPlanIds
        ? entity.hasPaidPlanIds.map((a) => MongoosePlanMapper.toDomain(a))
        : undefined,
  ...
  static toMongoose(entity: Coupon): Partial<CouponDocument> {
    ...
    return {
      ...
      doesNotHavePaidPlanIds: entity.doesNotHavePaidPlanIds
        ? (entity.doesNotHavePaidPlanIds.map(
            (plan) => new mongoose.Types.ObjectId(plan.id),
          ) as unknown as PlanDocument[])
        : undefined,
      hasPaidPlanIds: entity.hasPaidPlanIds
        ? (entity.hasPaidPlanIds.map(
            (plan) => new mongoose.Types.ObjectId(plan.id),
          ) as unknown as PlanDocument[])
        : undefined,
      expirationDate: entity.expirationDate,

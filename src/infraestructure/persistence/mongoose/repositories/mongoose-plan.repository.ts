...
export class MongoosePlanRepository implements IPlanRepository {
  ...
  constructor(
    @InjectModel(PlanMongoose.name)
    private readonly planModel: Model<PlanMongoose>,
  ...
  async findNotFreeDays(): Promise<Plan[]> {
    const query = await this.planModel
      .find({ freeTrialDays: { $exists: false }, discount: { $exists: false } })
      .sort({ price: 1 })
      .exec();

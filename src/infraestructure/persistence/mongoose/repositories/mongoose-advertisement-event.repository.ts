...
export class MongooseAdvertisementEventRepository
  implements IAdvertisementEventRepository
{
  constructor(
  ...
  async findOneByAdvertisementIdAndType(
    advertisementId: string,
    type: string,
  ): Promise<AdvertisementEvent> {
    const query = await this.advertisementEventModel
      .findOne({ advertisementId, type })
      .exec();
  ...
  async create(
    ...
    const data =
      MongooseAdvertisementEventMapper.toMongoose(advertisementEvent);
  ...
  async update(id: string, count: number): Promise<AdvertisementEvent> {
    const updated = await this.advertisementEventModel

...
export class MongooseAdvertisementCodeRepository
  implements IAdvertisementCodeRepository
{
  constructor(
    @InjectModel(AdvertisementCodeMongoose.name)
    private readonly advertisementCodeModel: Model<AdvertisementCodeMongoose>,
  ) {}

  async generateNewCode(): Promise<AdvertisementCode> {
    const updated = await this.advertisementCodeModel
      .findOneAndUpdate({}, { $inc: { code: 1 } }, { new: true, upsert: true })
      .exec();

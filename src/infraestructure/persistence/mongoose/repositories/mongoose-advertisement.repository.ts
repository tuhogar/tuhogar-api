...
export class MongooseAdvertisementRepository
  implements IAdvertisementRepository
{
  constructor(
    @InjectModel(AdvertisementMongoose.name)
    private readonly advertisementModel: Model<AdvertisementMongoose>,
    @InjectModel(AdvertisementEventMongoose.name)
    private readonly advertisementEventModel: Model<AdvertisementEventMongoose>,
  ) {}
  ...
  async findByAccountIdWithEvents(
    accountId: string,
    page: number,
    limit: number,
    code: number,
    ...
    const advertisementsWithAdvertisementEvents = advertisements.map(
      (advertisement) => {
    ...
    return {
      data: advertisementsWithAdvertisementEvents.map((item) =>
        MongooseAdvertisementMapper.toDomain(item),
      ),
      count,
    };
  }
  ...
  async findOneActive(advertisementId: string): Promise<Advertisement> {
    const query = await this.advertisementModel
      .findOne({ _id: advertisementId, status: AdvertisementStatus.ACTIVE })
      .populate('amenities')
      .populate('communityAmenities')
      .exec();
  ...
  async findToApprove(): Promise<Advertisement[]> {
    const query = await this.advertisementModel
      .find({ status: AdvertisementStatus.WAITING_FOR_APPROVAL })
      .populate('amenities')
      .populate('communityAmenities')
      .sort({ updatedAt: -1 })
      .exec();
  ...
  async updatePhotos(
    ...
    const updated = await this.advertisementModel
      .findOneAndUpdate({ accountId, _id: advertisementId }, update, {
        new: true,
      })
      .exec();
  ...
  async findOneById(id: string): Promise<Advertisement> {
    const query = await this.advertisementModel
      ...
      .exec();
    if (query) {
      const advertisementEvents = await this.advertisementEventModel
        .find({ advertisementId: query.id })
        .exec();
  ...
  async countActiveOrWaitingByAccountId(accountId: string): Promise<number> {
    const count = await this.advertisementModel
      .countDocuments({
   ...
   */
  async findActiveOrWaitingWithExcessPhotos(
    accountId: string,
    maxPhotos: number,
  ): Promise<Advertisement[]> {
    const advertisements = await this.advertisementModel

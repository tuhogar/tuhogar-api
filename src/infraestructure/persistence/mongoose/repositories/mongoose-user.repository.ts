...
export class MongooseUserRepository implements IUserRepository {
  ...
  async findByAccountIdAndUserRole(
    accountId: string,
    userRole?: UserRole,
  ): Promise<User[]> {
  ...
  async deleteFavoriteAdvertisement(
    id: string,
  ...
  async addFavoriteAdvertisement(
    id: string,
    advertisementId: string,
  ): Promise<User> {
    const updated = await this.userModel
  ...
  async findOneById(id: string): Promise<User> {
    const query = await this.userModel
      ...
      .populate({
        ...
        populate: [{ path: 'amenities' }, { path: 'communityAmenities' }],
      })
      .exec();
  ...
  async findByAccountId(accountId: string): Promise<User[]> {
    ...
    const usersWithSubscriptions = await Promise.all(
      users.map(async (user) => {
        ...
        const subscriptions = await this.userModel.aggregate([
          ...
          {
            $replaceRoot: {
              newRoot: { $mergeObjects: ['$subscriptions', '$$ROOT'] },
            },
          }, // Inclui todos os campos de subscriptions
    ...
    return usersWithSubscriptions.map((item) =>
      MongooseUserMapper.toDomain(item),
    );
  }
}

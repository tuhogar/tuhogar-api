...
export class MongooseAccountRepository implements IAccountRepository {
  constructor(
    @InjectModel(AccountMongoose.name)
    private readonly accountModel: Model<AccountMongoose>,
  ) {}

  async findActives(): Promise<Account[]> {
    const query = await this.accountModel
  ...
  async deleteImage(id: string): Promise<Account> {
    const updated = await this.accountModel
      .findOneAndUpdate({ _id: id }, { $unset: { photo: '' } }, { new: true })
      .exec();
  ...
  async updateImage(id: string, imageUrl: string): Promise<Account> {
    const updated = await this.accountModel
      .findOneAndUpdate({ _id: id }, { photo: imageUrl }, { new: true })
      .exec();
  ...
  async updateStatus(id: string, status: AccountStatus): Promise<Account> {
    const updated = await this.accountModel
      .findOneAndUpdate({ _id: id }, { status }, { new: false })
      .exec();
  ...
  async updatePlan(id: string, planId: string): Promise<Account> {
    const updated = await this.accountModel
      .findOneAndUpdate({ _id: id }, { $set: { planId } }, { new: true })
      .exec();
  ...
  async findOneByIdWithSubscription(id: string): Promise<Account> {
    ...
    const subscriptionsJSON =
      subscriptions[0]?.subscriptions?.map((sub) =>
  ...
  async updateHasPaidPlan(id: string, hasPaidPlan: boolean): Promise<Account> {
    const updated = await this.accountModel
      .findOneAndUpdate({ _id: id }, { $set: { hasPaidPlan } }, { new: true })
      .exec();
  ...
  async updatePaymentToken(id: string, paymentToken: string): Promise<Account> {
    const updated = await this.accountModel
      .findOneAndUpdate({ _id: id }, { $set: { paymentToken } }, { new: true })
      .exec();

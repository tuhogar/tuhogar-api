...
export class MongooseAccountCouponRepository
  implements IAccountCouponRepository
{
  constructor(
    @InjectModel(AccountCouponMongoose.name)
    private readonly accountCouponModel: Model<AccountCouponMongoose>,
  ) {}
  ...
  async findLastNotDepletedByAccountId(
    accountId: string,
  ): Promise<AccountCoupon> {
    const entity = await this.accountCouponModel
      .findOne({ accountId, isDepleted: false })
      .populate('couponId')
      .sort({ createdAt: -1 })
      .exec();
    return MongooseAccountCouponMapper.toDomain(entity);
  ...
  async findTypeDocumentCouponByAccountId(
    accountId: string,
  ): Promise<AccountCoupon | null> {
    const entity = await this.accountCouponModel
      .findOne({ accountId })
      .populate({
        path: 'couponId',
  ...
  async deplete(accountCouponId: string): Promise<void> {
    await this.accountCouponModel
      .updateOne({ _id: accountCouponId }, { isDepleted: true })
      .exec();
  }
}

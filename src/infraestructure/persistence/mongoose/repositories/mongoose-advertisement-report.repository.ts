...
export class MongooseAdvertisementReportRepository
  implements IAdvertisementReportRepository
{
  constructor(
    @InjectModel(AdvertisementReportMongoose.name)
    private readonly advertisementReportModel: Model<AdvertisementReportMongoose>,
  ) {}

  async findByAdvertisementId(
    advertisementId: string,
  ): Promise<AdvertisementReport[]> {
    const query = await this.advertisementReportModel
      .find({ advertisementId })
      .exec();
    return query.map((item) =>
      MongooseAdvertisementReportMapper.toDomain(item),
    );
  }
  ...
  async create(
    ...
    const data =
      MongooseAdvertisementReportMapper.toMongoose(advertisementReport);

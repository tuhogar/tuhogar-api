...
export class MongooseBulkUpdateDateRepository
  implements IBulkUpdateDateRepository
{
  constructor(
    @InjectModel(BulkUpdateDateMongoose.name)
    private readonly bulkUpdateDateModel: Model<BulkUpdateDateMongoose>,
  ) {}
  ...
  async update(bulkUpdateDate: BulkUpdateDate): Promise<BulkUpdateDate> {
    ...
    const updated = await this.bulkUpdateDateModel

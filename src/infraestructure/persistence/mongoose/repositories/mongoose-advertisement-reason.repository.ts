...
export class MongooseAdvertisementReasonRepository
  implements IAdvertisementReasonRepository
{
  constructor(
    @InjectModel(AdvertisementReasonMongoose.name)
    private readonly advertisementReasonModel: Model<AdvertisementReasonMongoose>,
  ) {}
  ...
  async find(): Promise<AdvertisementReason[]> {
    const query = await this.advertisementReasonModel
      .find()
      .sort({ name: 1 })
      .exec();
    return query.map((item) =>
      MongooseAdvertisementReasonMapper.toDomain(item),
    );
  }

  async create(
    advertisementReason: AdvertisementReason,
  ): Promise<AdvertisementReason> {
    const data =
      MongooseAdvertisementReasonMapper.toMongoose(advertisementReason);
  ...
  async update(
    ...
    const updated = await this.advertisementReasonModel
      .findOneAndUpdate(
        {
          _id: id,
        },
